...
const createInvoice = async (req, res) => {
    ...
    InvoiceOfItemToCreate = Joi.object({
        "Customer id": {uuid},
        "amount":"vat",
        "due_date": "Seller SA",           
...
const validateInvoiceBeforeCreate = InvoiceOfItemToCreate.validate(req.body)
...
const viewInvioce = (req, res) => {
    ...
    const viewInvoiceSaved = InvoiceOfItemToCreate() 
    if (viewInvoiceSaved.data.data != '')
...
const sendNotification = async (req, res) => {
    ...
    if (createInvoice != '')
        ...
        try{
            const SendNotification = await InvoiceOfItemToCreateInModel.getInvoiceByPhoneAndEmail(email, phone);
        }
        catch (err) {
            res.status(200).send({
                status: true,
